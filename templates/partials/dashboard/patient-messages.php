<?php
/**
 * Patient Messages Dashboard Template Part
 *
...
// Get messages grouped by dentist
global $wpdb;
...
$messages_query = $wpdb->prepare(
    ...
    LEFT JOIN {$wpdb->users} d ON m.dentist_id = d.ID 
    WHERE m.patient_id = %d 
        ...
        <div class="dental-messages-list">
            <?php if (empty($conversations)): ?>
                <div class="dental-empty-messages">
                    ...
                    <p>
                        <?php esc_html_e('No has enviado mensajes a ningún dentista todavía.', 'dental-directory-system'); ?>
                        <a href="<?php echo esc_url(add_query_arg('view', 'find-dentist', get_permalink())); ?>" class="dental-link">
                            <?php esc_html_e('Buscar dentistas ahora', 'dental-directory-system'); ?>
                        </a>
                    </p>
                </div>
            <?php else: ?>
                ...
                ?>
                    <div class="dental-message-item <?php echo $has_unread ? 'has-unread' : ''; ?>" data-conversation="<?php echo esc_attr($conversation->conversation_id); ?>" data-dentist="<?php echo esc_attr($dentist_id); ?>">
                        <div class="dental-message-avatar">
                            <?php echo get_avatar($dentist_id, 50); ?>
                        </div>
                        <div class="dental-message-details">
                            <div class="dental-message-header">
                                <span class="dental-message-sender"><?php echo esc_html($conversation->dentist_name); ?></span>
...
jQuery(document).ready(function($) {
    ...
    $('.dental-message-item').on('click', function() {
        ...
        const dentistId = $(this).data('dentist');
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.success) {
                    ...
                    initMessageReply(conversationId, dentistId);
                } else {
    ...
    function initMessageReply(conversationId, dentistId) {
